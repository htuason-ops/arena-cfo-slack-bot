// Renders the Arena "Meet Your CFO" flyer headlessly and returns { png, pdf }
// buffers, using the same template that a human would fill out in the
// browser (./template/flyer.html, right next to this file), driven by the
...
//
// Browser: this uses puppeteer-core + @sparticuz/chromium instead of the
// full `puppeteer` package. The full package tries to *download* a Chrome
// binary during `npm install`, which is exactly what caused the repeated
// "Could not find Chrome" errors on Render (build-time download landing in
// a cache path the runtime couldn't see, or not running at all). chromium
// ships its own Chromium build as a regular npm package asset instead — no
// separate download step, no cache-path guessing.
// Source: https://www.npmjs.com/package/@sparticuz/chromium
